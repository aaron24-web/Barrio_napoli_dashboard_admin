...
const promotionsData = [
  { id: 1, name: 'Pizza 2x1', description: 'Compra una pizza y llévate la segunda gratis', type: 'Oferta Especial', conditions: 'Aplica solo en pizzas seleccionadas', startDate: new Date(2024, 10, 1), endDate: new Date(2024, 10, 15), availability: true, products: [1, 2] },
  { id: 2, name: 'Combo Amigos', description: '2 pizzas grandes + 1 refresco de 2L', type: 'Combo', conditions: 'No aplica con otras promociones', startDate: new Date(2024, 10, 1), endDate: new Date(2024, 10, 31), availability: true, products: [1, 2, 4] },
  { id: 3, name: 'DTO10', description: '10% de descuento en tu compra total', type: 'Cupón', conditions: 'Válido solo en compras mayores a $200', startDate: new Date(2024, 11, 1), endDate: new Date(2024, 11, 31), availability: false, products: [] },
  { id: 4, name: '20% en Pizzas', description: '20% de descuento en todas las pizzas', type: 'Descuento', conditions: 'No aplica con otras promociones', startDate: new Date(2024, 11, 1), endDate: new Date(2024, 11, 15), availability: true, products: [1, 2, 3], discount: 20 },
]
...
export function Promotions() {
  ...
  const paginatedPromotions = filteredPromotions.slice((page - 1) * promotionsPerPage, page * promotionsPerPage)
  ...
  function handleAvailabilityChange(id: number, availability: boolean) {
    setPromotions(promotions.map(p => p.id === id ? { ...p, availability } : p))
  }
    ...
    <>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button onClick={() => setEditingPromotion(null)}>Crear Promoción</Button>
            </DialogTrigger>
        ...
        <div className="flex items-center gap-2">
          <Input placeholder="Buscar promociones..." value={search} onChange={e => setSearch(e.target.value)} />
                ...
                <TableRow key={promotion.id}>
                  ...
                  <TableCell>
                    <Switch
                      ...
                      onCheckedChange={(checked) => handleAvailabilityChange(promotion.id, checked)}
                    />
                  </TableCell>
                  <TableCell>
                    <Button variant="outline" size="sm" className="mr-2" onClick={() => { setEditingPromotion(promotion); setIsDialogOpen(true); }}>Editar</Button>
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={() => handleDeletePromotion(promotion.id)}>Eliminar</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  </TableCell>
