...
test('paginar pedidos', async ({ page }) => {
  await page.goto('/orders', { waitUntil: 'networkidle' })
...
test('filtrar por id de pedido', async ({ page }) => {
  await page.goto('/orders', { waitUntil: 'networkidle' })

  await page.getByPlaceholder('ID del pedido').fill('order-11')
  await page.getByRole('button', { name: 'Filtrar resultados' }).click()

  expect(page.getByRole('cell', { name: 'order-11' })).toBeVisible()
})

test('filtrar por nombre de cliente', async ({ page }) => {
  await page.goto('/orders', { waitUntil: 'networkidle' })

  await page.getByPlaceholder('Nombre del cliente').fill('Customer 11')
  await page.getByRole('button', { name: 'Filtrar resultados' }).click()
...
test('filtrar por estado', async ({ page }) => {
  ...
  await page.getByLabel('Pendiente').click()

  await page.getByRole('button', { name: 'Filtrar resultados' }).click()

  const tableRows = await page.getByRole('cell', { name: 'Pendiente' }).all()
